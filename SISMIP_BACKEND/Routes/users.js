...
router.get('/', auth, async (req, res) => {
  try {
    ...
    if (user.length == 0)
      return res.send({ message: 'Nenhum Usuário Cadastrado' });
...
router.post('/delete', auth, async (req, res) => {
  ...
  if (name == 'admin' || email == 'admin')
    return res
      .status(403)
      .send({ message: 'Não é Possível Excluir o Administrador' });

  try {
    var user = await User.deleteOne({ email });

    if (user.deletedCount == 0)
      return res.status(410).send({ message: 'Usuário Inexistente' });
...
router.post('/update', auth, async (req, res) => {
  ...
  if (name == 'admin' || email == 'admin')
    return res
      .status(403)
      .send({ message: 'Não é Possível Alterar o Administrador' });

  if (!email || !name || !password)
    return res
      .status(403)
      .send({ message: 'Dados inseridos invalidos e/ou insuficientes' });

  try {
    var user = await User.updateOne(
      { email },
      { $set: { name: name, password: encryptedPassword(password) } }
    );
